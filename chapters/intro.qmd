...
Finalmente, la marca RStudio se está convirtiendo en [Posit](https://posit.co/).
...
En R se programa. Aprender a programar es un proceso de largo aliento, no es como aprender a usar una suite de bussines analytics como Power BI.[^intro-2] Puede llevar meses sentirse a gusto con la programación. Lo fundamental es la práctica. Felizmente, no necesitamos ser expertos en programación para realizar análisis de datos, sino que tan solo tenemos que saber usar las librerías y funciones adecuadas.
...
En el script anterior, definí una función llamada "mi_funcion", que utiliza a su vez otra función ya definida por R llamada *print* que sirve para publicar texto en la consola. Esta función lo único que hace es que al activarla, aparece el texto que está entre comillas en la consola. Si yo le cambio ese texto y vuelvo a definir la función, cambiará el resultado.

```{r}
mi_funcion()
```

En este ejemplo claramente no se ven las funciones como algo que ayude mucho, pero obviamente para realizar análisis de datos se utilizan unas mucho más sofisticadas, desarrolladas por especialistas. Nosotros como analistas podemos crear las nuestras, pero seguramente sea necesario bajo determinados contextos. Lo ideal es conocer cómo funcionan. Otro elemento central en las funciones son los parámetros.

## Parámetros

Los parámetros son valores que se sustituirán por aquellos que necesitemos al llamar a la función. Sirven para generar funciones que se adapten a cambios en las necesidades del analista. En este caso, se crea una función *nombre_completo* que recibe dos parámetros: *nombre* y *apellido.* Luego, con otra función de R llamada *paste* que lo hace es transformar lo que tiene adentro a texto y unirlo, une los dos valores que ingresé cómo parámetros y los muestra en consola.
...
Las funciones suelen tener nombres que explicitan lo que hacen.

```{r}
nombre_completo("Homero", "Simpson")
```

Si hubiéramos usado parámetros en nuestra función anterior:

```{r}
mi_funcion_con_params <- function(texto) { 
  print(texto)
}

mi_funcion_con_params("Hola, Mundo!")
mi_funcion_con_params("Hola, Universo!")
```
